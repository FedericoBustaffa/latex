...
\end{enumerate}
In questo caso si fanno $h$ tentativi e la probabilit\`a di fallire \`e di $1/2$ ogni volta che si traduce in una
probabilit\`a di circa $(1/2)^h$ di fallimento totale. Se $h$ \`e sufficientemente grande, la probabilit\`a di
fallire \`e molto bassa.