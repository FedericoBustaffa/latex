...
\begin{definition}
	Si dice che una certa ipotesi $h_j$ \`e \textbf{pi\`u generale} di $h_k$ se e solo se per ogni $x \in X$, dove $X$
	\`e l'insieme delle variabili del problema, vale che
...
Nello specifico vedremo un metodo in cui si analizzer\`a prima l'ipotesi pi\`u specifica, arrivando poi a quella pi\`u
generale.

Si \emph{generalizzano} quindi, le ipotesi pi\`u specifiche in maniera \textbf{conservativa}, ovvero facendo s\`i che, per
ogni esempio positivo, anche la $h$ sia positiva in modo che sia anche \emph{consistente} con il training set.
...
\subsubsection{Algoritmo di eliminazione con lista}
...
\subsubsection{Algoritmo di eliminazione per esempi positivi}
...
\section{Perch\'e bias induttivi}
