...
\begin{enumerate}
	\item Il KDC condivide con l'utente $A$ una chiave $k_A$.
	\item Il KDC invia informazioni cifrate con la chiave $k_A$ ad $A$ per interagire con il TGS.
	\item L'utente $A$ decifra le informazioni e le invia al TGS.
	\item Il TGS fornisce un ticket di validità temporanea ad $A$ con il quale può autenticarsi con il server ed essere
	      quindi in grado di usufruire dei suoi servizi.
\end{enumerate}