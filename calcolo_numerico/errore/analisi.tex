...
In un'\textbf{analisi all'indietro} si assume che la funzione calcolata non sia $g(\tx)$ ma che sia $f(\hat{x})$,
ossia la funzione
i dati in ingresso siano numeri di macchina e che quindi non
...
accumulabile. Vediamo come cambiano il grafo e dunque il calcolo dell'errore per l'algoritmo $\frac{x-1}{x}$.
\begin{center}
	\begin{tikzpicture}[mynode/.style={draw=red, thick, circle, font=\Large}]
