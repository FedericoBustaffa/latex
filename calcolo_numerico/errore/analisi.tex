...
\subsection{Calcolo dell'errore totale mediante grafi}
Per calcolare l'errore totale non dobbiamo fare altro che aggiungere l'errore di rappresentazione sui dati in
ingresso e procedere nel solito modo. Facciamolo per l'algoritmo $\frac{x-1}{x}$.
\begin{center}
	\begin{tikzpicture}[mynode/.style={draw=red, thick, circle, font=\Large}]
		...
		(1) edge[thick] node[below, black] {$-\frac{1}{x-1}$} (x-1)
		(x-1) edge[thick] node[above, black] {$1$} (x-1/x)
		(x2) edge[thick] node[below, black] {$(-1)$} (x-1/x);
	\end{tikzpicture}
\end{center}