...
\begin{theorem}
	...
	\begin{proof}
		...
		\[
			...
			\end{bmatrix}
		\]
		Come prima vogliamo dimostrare la predominanza diagonale di tale matrice per $|\lambda| \geq 1$, per farlo
		scriviamo
		\[ |\lambda a_{i,i}| > \sum_{j=1}^{j=i-1} |\lambda a_{i,j}| + \sum_{j=i+1}^n |a_{i,j}| \]
	\end{proof}
\end{theorem}
