...
In questo caso $P_X$ non è determinata dalla sua densità ma dall'integrale della densità. In
generale la funzione di densità ci dice come il carattere è distribuito e non ci deve stupire se
questa assume valori anche maggiori di 1 in quanto la densità \textbf{non} è una probabilità.

\begin{example}
	Estraiamo un individuo $\omega$ da una popolazione $\Omega$ e consideriamo
	\begin{itemize}
		\item $X(\omega)$ che equivale al numero di figli di $\omega$ (carattere discreto).
		\item $Y(\omega)$ che equivale all'altezza di $\omega$ (carattere continuo).
	\end{itemize}
	Abbiamo che
	\[ P_X(x_i) = P(X = x_i) = \frac{\# \{ \omega | X(\omega) = x_i\} }{\# \Omega} \]
	ossia la frequenza relativa di $X = x_i$ su tutta la popolazione. $P_X(2)$ è quindi la
	frequenza relativa, sulla popolazione italiana, delle persone con 2 figli. Per quanto riguarda
	$Y$ invece, abbiamo che l'area sottesa da $f$ nell'intervallo $[a,b]$ equivale a
	\[
